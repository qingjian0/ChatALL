...
function safeRequireElectron() {
  try {
    ...
    console.warn('[Web Access] Electron not available');
  }
  return null;
}
...
const ipcRenderer = electron?.ipcRenderer || {
  invoke: async () => ({ shouldUseDarkColors: false }),
  on: () => {},
  send: () => {},
};
...
function toggleAppBar() {
  ...
  if (electron) {
    ipcRenderer.invoke('set-is-show-menu-bar', isShowAppBar.value);
  }
}
