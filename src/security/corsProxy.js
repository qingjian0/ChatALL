...
class CorsProxy {
      ...
      try {
        ...
        if (!this.isTargetAllowed(targetUrl.hostname)) {
          ...
          validation.errors.push(
            `Target host not allowed: ${targetUrl.hostname}`,
          );
        }
      } catch {
  ...
  recordProxyUsage(proxyName, success) {
    ...
    if (proxy) {
      ...
      if (!success) {
        ...
        if (
          this.circuitBreaker.failCount >= this.circuitBreaker.failThreshold
        ) {
  ...
  isCircuitBreakerTripped() {
    ...
    const now = Date.now();
    if (
      now - this.circuitBreaker.lastFailTime >=
  ...
  getProxyStats() {
    ...
    return {
      ...
      rateLimitedRequests: recentRequests.filter((r) => r.statusCode === 429)
        .length,
  ...
  setRateLimit(limit, windowMs = DEFAULT_RATE_WINDOW) {
    ...
    logInfo(
      "[CORS] Rate limit updated:",
      `${limit} requests/${windowMs / 1000}s`,
    );
  }
