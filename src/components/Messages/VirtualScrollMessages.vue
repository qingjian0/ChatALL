...
const visibleStartIndex = computed(() => {
  const start =
    Math.floor(scrollTop.value / estimatedItemHeight.value) - buffer;
...
const visibleEndIndex = computed(() => {
  ...
  const end =
    Math.floor(
      (scrollTop.value + containerHeightNum) / estimatedItemHeight.value,
    ) + buffer;
...
function getItemOffset(item) {
  ...
  for (let i = 0; i < itemIndex; i++) {
    offset +=
      itemHeights.value.get(props.messages[i].id) || estimatedItemHeight.value;
  }
  return offset;
}
