  ...
  <div class="cors-proxy-settings">
        ...
        <v-row>
          ...
          <v-col cols="12" sm="6" md="4">
            ...
            <span class="text-body-1 font-medium ml-2">
              {{ activeProxyName || $t('proxy.none') }}
            </span>
          </v-col>
          <v-col cols="12" sm="6" md="4">
            <span class="text-body-1">{{ $t('proxy.customProxies') }}:</span>
            <span class="text-body-1 font-medium ml-2">{{ customProxies.length }}</span>
          </v-col>
        </v-row>
      </v-card-text>
    </v-card>
    ...
    <v-card class="mb-4">
      ...
      <v-card-text>
        <v-list>
          <v-list-item>
            <v-list-item-title>{{ $t('proxy.enableCorsProxy') }}</v-list-item-title>
            <v-switch
          ...
          <v-list-item>
            <v-list-item-title>{{ $t('proxy.autoFallback') }}</v-list-item-title>
            <v-switch
    ...
    <v-card class="mb-4">
      ...
      <v-card-text>
          ...
          >
            <v-list-item-avatar>
              <v-icon :color="getProxyStatusColor(proxy)">
                {{ proxy.name === activeProxyName ? 'mdi-check-circle' : 'mdi-circle' }}
              </v-icon>
            </v-list-item-avatar>
            <v-list-item-content>
              <v-list-item-title class="font-medium">{{ proxy.name }}</v-list-item-title>
              <v-list-item-subtitle class="text-grey">{{ proxy.url }}</v-list-item-subtitle>
              <v-row class="mt-2">
                <v-chip size="small" :color="getProxyHealthColor(proxy)">
                  {{ getProxyHealthStatus(proxy) }}
                </v-chip>
        ...
        <v-empty-state v-else :icon="'mdi-proxy'" :title="$t('proxy.noProxies')">
          <template #actions>
    ...
    <v-card class="mb-4">
      ...
      <v-card-text>
        <v-list>
          <v-list-item
            v-for="proxyName in fallbackProxies"
            :key="proxyName"
          >
        ...
        <v-row>
          <v-col cols="12" sm="6" md="3">
            <v-card class="text-center">
              ...
              <v-card-text class="text-body-2 text-grey">{{ $t('proxy.totalRequests') }}</v-card-text>
            </v-card>
          </v-col>
          <v-col cols="12" sm="6" md="3">
            <v-card class="text-center">
              ...
              <v-card-text class="text-body-2 text-grey">{{ $t('proxy.requestsLastHour') }}</v-card-text>
            </v-card>
          </v-col>
          <v-col cols="12" sm="6" md="3">
            <v-card class="text-center">
              ...
              <v-card-text class="text-body-2 text-grey">{{ $t('proxy.rateLimited') }}</v-card-text>
            </v-card>
          </v-col>
          <v-col cols="12" sm="6" md="3">
            <v-card class="text-center">
              ...
              <v-card-text class="text-body-2 text-grey">{{ $t('proxy.errors') }}</v-card-text>
            </v-card>
          </v-col>
        </v-row>
      </v-card-text>
    ...
    <!-- 添加代理对话框 -->
    <v-dialog v-model="showAddProxyDialog" max-width="500px">
      <v-card>
        ...
        <v-card-actions>
          <v-btn @click="showAddProxyDialog = false">{{ $t('common.cancel') }}</v-btn>
          <v-btn color="primary" @click="addProxy" :disabled="!addProxyForm.valid">
            {{ $t('common.add') }}
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <!-- 备用代理选择器 -->
    <v-dialog v-model="showFallbackSelector" max-width="500px">
      <v-card>
        ...
        <v-card-actions>
          <v-btn @click="showFallbackSelector = false">{{ $t('common.close') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
const availableFallbackProxies = computed(() => {
 return customProxies.value.filter((p) => !fallbackProxies.value.includes(p.name));
});
onMounted(() => {
 loadSettings();
 loadProxies();
...
async function testProxy(proxy) {
 ...
 if (result.valid) {
 showSnackbar($t('proxy.testSuccess', { code: result.responseCode }), 'success');
 }
 else {
