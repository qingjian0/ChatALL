...
export const mockIpcs = {
  on(channel, handler) {
    ...
    return () => {
      ipcEventHandlers[channel] = ipcEventHandlers[channel].filter(h => h !== handler)
    }
  },
...
export const mockShell = {
  ...
  writeText(text) {
    console.log('[Mock Shell] writeText:', text?.slice(0, 50) + (text?.length > 50 ? '...' : ''))
    return Promise.resolve()
  },
...
export const mockDialog = {
  showMessageBox(options) {
    ...
    mockDialogs.push({ type: 'messageBox', options, response: { response: 0, checkboxChecked: false } })
    return Promise.resolve({ response: 0, checkboxChecked: false })
  },

  showOpenDialog(options) {
    console.log('[Mock Dialog] showOpenDialog')
    mockDialogs.push({ type: 'openDialog', options, response: { canceled: true, filePaths: [] } })
    return Promise.resolve({ canceled: true, filePaths: [] })
  },

  showSaveDialog(options) {
    console.log('[Mock Dialog] showSaveDialog')
    mockDialogs.push({ type: 'saveDialog', options, response: { canceled: true, filePath: null } })
