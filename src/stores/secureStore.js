...
export const useSecureStore = defineStore('secure', () => {
  ...
  const isAuthenticated = computed(() => isInitialized.value && !isLocked.value)
  ...
  async function importPublicKey(pem) {
    const binary = new Uint8Array(atob(pem).split('').map(c => c.charCodeAt(0)))
  ...
  async function decryptWithPrivateKey(privateKey, encryptedData) {
    const binary = new Uint8Array(atob(encryptedData).split('').map(c => c.charCodeAt(0)))
    const decrypted = await crypto.subtle.decrypt(
      { name: 'RSA-OAEP' },
      privateKey,
      binary
  ...
  function setSecurityLevel(level) {
    ...
    if (validLevels.includes(level)) {
      ...
      if (level === 'high' || level === 'maximum') {
        keyDerivationParams.value.iterations = level === 'maximum' ? 200000 : 150000
      }
    }
  }
  ...
  function secureLocalStorageClear() {
    const keys = Object.keys(localStorage).filter(k => k.startsWith('chatall-secure-'))
