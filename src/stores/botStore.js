...
export const useBotStore = defineStore('bot', () => {
  ...
  const offlineBots = computed(() => {
    return activeBots.value.filter(bot => {
      const status = botStatus.value[bot.classname]
      return status === 'offline' || status === 'disconnected' || status === 'error'
    })
  })
  ...
  function getDefaultBotConfigs() {
    return {
      geminiApi: { apiKey: '', temperature: 0.7, pastRounds: 5, topK: 16, topP: 0.95 },
      ...
      azureOpenaiApi: { apiKey: '', temperature: 1, pastRounds: 5, instanceName: '', deploymentName: '', apiVersion: '' },
      groqApi: { apiKey: '', temperature: 0, maxTokens: 1000, pastRounds: 1 },
      mistral: { model: 'mistral-large' },
      kimi: { accessToken: '', refreshToken: '' },
      doubao: { apiKey: '', modelId: '', baseUrl: '', temperature: 0.7, pastRounds: 5 },
      deepseek: { apiKey: '', modelId: 'deepseek-chat', baseUrl: '', temperature: 0.7, pastRounds: 5 },
      chatglmApi: { apiKey: '', modelId: 'glm-4', baseUrl: '', temperature: 0.7, pastRounds: 5 },
      qwenApi: { apiKey: '', modelId: 'qwen-plus', baseUrl: '', temperature: 0.7, pastRounds: 5 },
      sparkApi: { apiKey: '', modelId: 'generalv3.5', baseUrl: '', temperature: 0.7, pastRounds: 5 },
      miniMaxApi: { apiKey: '', modelId: 'abab6.5s-chat', baseUrl: '', temperature: 0.7, pastRounds: 5 },
  ...
  function getApiKey(botType) {
    ...
    return config?.apiKey || config?.accessToken || config?.token || config?.secretKey || ''
  }
      ...
      for (const [path, module] of Object.entries(botModules)) {
        try {
          ...
          if (botClass && botClass._model) {
            botList.push({
              class: botClass,
              classname: botClass.getClassname?.() || path.split('/').pop().replace('.js', ''),
  ...
  async function createBotInstance(botClassname) {
    ...
    try {
      ...
      console.error(`Failed to create bot instance for ${botClassname}:`, error)
      return null
    }
  }
  ...
  async function checkAllBotsOnlineStatus() {
    const promises = activeBots.value.map(bot => checkBotOnlineStatus(bot.classname))
  ...
  async function testBotConnection(botClassname) {
    const instance = getBotInstance(botClassname) || await createBotInstance(botClassname)
    if (!instance) {
