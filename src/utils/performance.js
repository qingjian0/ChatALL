...
export function markAppReady() {
  ...
  metrics.appStart.duration =
    metrics.appStart.readyTime - metrics.appStart.startTime;
}
...
export function getMetricsSummary() {
  ...
  const successRate =
    metrics.botRequests.length > 0
      ? metrics.botRequests.filter((r) => r.success).length /
        metrics.botRequests.length
      : 0;
...
export function logMetrics() {
  ...
  console.table &&
    console.table({
      ...
      "Memory Peak": summary.memory.peak
        ? `${summary.memory.peak.toFixed(2)}MB`
        : "N/A",
